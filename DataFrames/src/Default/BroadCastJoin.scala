...
object BroadCastJoin extends App{
  ...
  val ordersDf = spark.read
    ...
    val rrr = ordersDf.rdd
    
    val ty = mylist.toDF()
  
    val ss = spark.sparkContext.parallelize(mylist)
    
    ss.collect().foreach(println)
    
    
    customersDf.rdd.partitions.length
    customersDf.rdd.partitions.size
    customersDf.rdd.getNumPartitions
...
//    val join = "left"
//    
//    ordersDf.join(broadcast(customersDf),joinCondition, join).show
//    
