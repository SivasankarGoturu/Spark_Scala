...
object tyty extends App{
  ...
  val spark = SparkSession.builder()
    ...
    val avgPriceyDf = spark.read
      ...
      val joinType = "left"
      val joinCondition1 = df5.col("council") === salesOpt.col("council")
      val joinCondition2 = avgOpt.col("council") === salesOpt.col("council")
      
      val joinDf = df5.join(salesOpt, joinCondition1, joinType).join(avgOpt,joinCondition2, joinType)
      
      joinDf.select(df5.col("council"), df5.col("county"), df5.col("council_type"), col("avg_price_nov_2019"), col("sales_volume_sept_2019")).show()
